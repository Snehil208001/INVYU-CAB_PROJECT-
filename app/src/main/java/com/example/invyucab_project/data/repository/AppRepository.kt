...
class AppRepository @Inject constructor(
    ...
    suspend fun saveUserToFirestore(userId: Int, phoneNumber: String) {
        // ✅ Guard: Don't try to write if we aren't "active" locally,
        // although usually this is called explicitly after login.
        withContext(Dispatchers.IO) {
            try {
                ...
                // Log but don't crash. This often happens if permissions are missing.
                Log.e("AppRepository", "❌ Failed to sync user to Firestore: ${e.message}")
            }
        }
    }
    ...
    suspend fun syncCurrentUserToFirestore() {
        ...
        // ✅ FIX: Only sync if user is explicitly ACTIVE
        if (userId != null && !phone.isNullOrEmpty() && status == "active") {
    ...
    fun syncFcmToken() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                // ✅ FIX: Check if user is ACTIVE before trying to sync token
                // This prevents "Missing Authentication Token" errors during login/onboarding
                val status = userPreferencesRepository.getUserStatus()
                val phoneNumber = userPreferencesRepository.getPhoneNumber()

                if (status == "active" && !phoneNumber.isNullOrEmpty()) {
    ...
    suspend fun checkUser(phoneNumber: String): Response<CheckUserResponse> {
        ...
        // ❌ REMOVED: saveUserToFirestore()
        // We cannot save to Firestore here because the user is not authenticated yet.
        // The save will happen in OtpViewModel after successful sign-in.
    ...
    suspend fun createUser(request: CreateUserRequest): CreateUserResponse {
        ...
        // This is safe because createUser is called AFTER Firebase Auth in OtpViewModel
