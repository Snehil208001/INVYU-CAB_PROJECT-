        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (response.isSuccessful && response.body()?.success == true) {
                    ...
                    // Check if the response body is null (often happens with 0 items)
                ...
                // ✅ FIX: Handle JSON parsing errors for empty history
                // If backend sends empty body or missing fields when 0 rides exist,
                // Moshi throws "Required value 'success' missing".
                if (msg.contains("Required value 'success' missing") || msg.contains("End of input")) {
                    _rideHistory.value = emptyList()
                    _error.value = null // Clear error so UI shows "No rides"
                } else {
                    _error.value = msg
                }
            } finally {
