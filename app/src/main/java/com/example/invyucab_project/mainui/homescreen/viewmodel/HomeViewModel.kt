...
class HomeViewModel @Inject constructor(
    ...
    private val _serviceType = MutableStateFlow("Ride")
    ...
    // ✅ UPDATED: Now uses RiderRideHistoryItem
    private var cachedRideHistory: List<RiderRideHistoryItem> = emptyList()
    ...
    fun onServiceTypeChanged(type: String) {
        ...
        onClearDrop()
        filterAndShowRecentRides()
    }

    fun onScheduleTimeSelected(timestamp: Long?) {
        _scheduledTime.value = timestamp
    }
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.isSuccessful && response.body()?.success == true) {
                    cachedRideHistory = response.body()?.data ?: emptyList()
                    filterAndShowRecentRides()
                }
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    private fun filterAndShowRecentRides() {
        val currentService = _serviceType.value
        val isCourierTab = currentService == "Courier"

        viewModelScope.launch {
            val filteredList = cachedRideHistory.filter { item ->
                val itemIsCourier = item.isCourier == true
                itemIsCourier == isCourierTab
            }

            val recentItems = filteredList.reversed().take(3).map { item ->
    ...
    fun onRecentRideClicked(ride: RecentRide) {
        viewModelScope.launch {
            val isCourier = _serviceType.value == "Courier"
    ...
    fun onContinueClicked() {
        ...
        if (!currentState.pickupPlaceId.isNullOrBlank() && !currentState.dropPlaceId.isNullOrBlank()) {
            ...
            val sTime = _scheduledTime.value

            viewModelScope.launch {
                if (service == "Courier") {
                    sendEvent(
                        UiEvent.Navigate(
                            Screen.CourierDetailsScreen.createRoute(
                                pickupPlaceId = currentState.pickupPlaceId!!,
                                pickupDesc = pickupDesc,
                                dropPlaceId = currentState.dropPlaceId!!,
                                dropDesc = currentState.dropQuery,
                                scheduleTime = sTime
                            )
                        )
                    )
