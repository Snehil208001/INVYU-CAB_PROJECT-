...
class RideSelectionViewModel @Inject constructor(
    ...
    // --- Trip Scheduling & Courier Params ---
    private val scheduleTime: Long = savedStateHandle.get<Long>("scheduleTime") ?: 0L
    private val isCourier: Boolean = savedStateHandle.get<Boolean>("isCourier") ?: false
    private val receiverName: String = decodeUrlString(savedStateHandle.get<String>("receiverName") ?: "")
    private val receiverPhone: String = decodeUrlString(savedStateHandle.get<String>("receiverPhone") ?: "")
    private val packageType: String = decodeUrlString(savedStateHandle.get<String>("packageType") ?: "")
    ...
    init {
        ...
        // ✅ CORRECTED: Initialize appropriate options for Courier vs Ride
    ...
    fun getFormattedScheduleTime(): String? {
        return if (scheduleTime > 0) {
            SimpleDateFormat("EEE, d MMM hh:mm a", Locale.getDefault()).format(Date(scheduleTime))
        } else null
    }

    fun isCourierBooking(): Boolean = isCourier
    ...
    private fun initializeRideOptions() {
        val options = if (isCourier) {
            // ✅ COURIER: Only Bike and Auto, with capacity descriptions
            listOf(
                RideOption(1, Icons.Default.TwoWheeler, "Moto Package", "Small items (Max 10kg)", null),
                RideOption(2, Icons.Default.ElectricRickshaw, "Auto Package", "Larger items (Max 20kg)", null)
            )
        } else {
            // ✅ RIDE: Standard options
            listOf(
                RideOption(1, Icons.Default.TwoWheeler, "Bike", "Fast & Affordable", null),
                RideOption(2, Icons.Default.ElectricRickshaw, "Auto", "Reliable & Spacious", null),
                RideOption(3, Icons.Default.LocalTaxi, "Car", "Comfortable AC Ride", null)
            )
        }
    ...
    fun onBookRideClicked(selectedRideId: Int) {
        ...
        val bookingStatus = if (scheduleTime > 0) "scheduled" else "requested"
        ...
        val request = CreateRideRequest(
            ...
            scheduledTime = if (scheduleTime > 0) scheduleTime else null,
            isCourier = isCourier,
            receiverName = if (isCourier) receiverName else null,
            receiverPhone = if (isCourier) receiverPhone else null,
            packageType = if (isCourier) packageType else null
        )
