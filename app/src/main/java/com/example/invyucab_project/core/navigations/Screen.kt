...
sealed class Screen(val route: String) {
    ...
    object CourierDetailsScreen :
        Screen("courier_details_screen?pickupPlaceId={pickupPlaceId}&pickupDesc={pickupDesc}&dropPlaceId={dropPlaceId}&dropDesc={dropDesc}&scheduleTime={scheduleTime}") {
        fun createRoute(
            ...
            val pId = URLEncoder.encode(pickupPlaceId, StandardCharsets.UTF_8.toString())
            val pDesc = URLEncoder.encode(pickupDesc, StandardCharsets.UTF_8.toString())
            val dId = URLEncoder.encode(dropPlaceId, StandardCharsets.UTF_8.toString())
            val dDesc = URLEncoder.encode(dropDesc, StandardCharsets.UTF_8.toString())
            val sTime = scheduleTime ?: 0L
            return "courier_details_screen?pickupPlaceId=$pId&pickupDesc=$pDesc&dropPlaceId=$dId&dropDesc=$dDesc&scheduleTime=$sTime"
        }
    }

    object RideSelectionScreen :
        Screen("ride_selection_screen?dropPlaceId={dropPlaceId}&dropDescription={dropDescription}&pickupPlaceId={pickupPlaceId}&pickupDescription={pickupDescription}&pickupLat={pickupLat}&pickupLng={pickupLng}&dropLat={dropLat}&dropLng={dropLng}&isCourier={isCourier}&receiverName={receiverName}&receiverPhone={receiverPhone}&packageType={packageType}&scheduleTime={scheduleTime}") {
        fun createRoute(
            dropPlaceId: String,
            ...
            val encRecName = URLEncoder.encode(receiverName, StandardCharsets.UTF_8.toString())
            val encRecPhone = URLEncoder.encode(receiverPhone, StandardCharsets.UTF_8.toString())
            val encPkgType = URLEncoder.encode(packageType, StandardCharsets.UTF_8.toString())

            return "ride_selection_screen?dropPlaceId=$encodedDropId&dropDescription=$encodedDropDesc&pickupPlaceId=$encodedPickupId&pickupDescription=$encodedPickupDesc&pickupLat=$pLat&pickupLng=$pLng&dropLat=$dLat&dropLng=$dLng&isCourier=$isCourier&receiverName=$encRecName&receiverPhone=$encRecPhone&packageType=$encPkgType&scheduleTime=$sTime"
        }
    }
    ...
    object RideInProgressScreen : Screen("ride_in_progress_screen/{rideId}/{dropLat}/{dropLng}/{role}?otp={otp}&targetPhone={targetPhone}") {
        fun createRoute(
            rideId: Int,
            dropLat: Double,
            dropLng: Double,
            ...
            return "ride_in_progress_screen/$rideId/${dropLat.toFloat()}/${dropLng.toFloat()}/$role?otp=$otp&targetPhone=$tPhone"
        }
    }

    object BillScreen : Screen("bill_screen/{fare}/{role}?pickupAddress={pickupAddress}&dropAddress={dropAddress}") {
        fun createRoute(
            fare: String,
            role: String,
            pickupAddress: String?,
            ...
            val encodedPickup = pickupAddress?.let { URLEncoder.encode(it, StandardCharsets.UTF_8.toString()) } ?: ""
            val encodedDrop = dropAddress?.let { URLEncoder.encode(it, StandardCharsets.UTF_8.toString()) } ?: ""
            return "bill_screen/$fare/$role?pickupAddress=$encodedPickup&dropAddress=$encodedDrop"
        }
    }
